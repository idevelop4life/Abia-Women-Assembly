...
router.post("/facebook", async (req, res) => {
  ...
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ error: "Email permission not granted by Facebook" });
    }
...
router.get("/verify", authorization, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT first_name, last_name, email, primary_phone, nationality, state_city, profile_picture, is_verified
       FROM members 
       WHERE id = $1`,
      [userId],
    );

    console.error("User verification result:", result.rows);
